...
Window_BattleSkill.prototype.drawAllItems = function() {
	//this.drawPic('battleUse', 1, 0-20,924,180);
    var topIndex = this.topIndex();
    for (var i = 0; i < this.maxPageItems(); i++) {
        var index = topIndex + i;
        if (index < this.maxItems()) {
	...
	if(this._actor){
		...
		this.changeTextColor(this.normalColor());
		this.drawText('新卡组：'+$gameParty._cards.length,840,50)
		this.drawText('废卡组：'+$gameParty._Qcards.length,840,75)
		var nm = this._actor.mp
		for(var i = 0;i < nm;i++){
			this.drawIcon(165, 840+i*48, 180);
		}
		this.drawActorFace(this._actor, 1100-16, 24);
	}
};
Window_BattleSkill.prototype.makeItemList = function() {
	this._data = [];
	for(var i = 0;i<$gameParty._Gcards.length;i++){
		this._data.push($dataSkills[$gameParty._Gcards[i]])
	}
};
...
Window_BattleSkill.prototype.drawItem = function(index) {
    ...
    if (skill) {
        ...
        //rect.width -= this.textPadding();
        this.changePaintOpacity(this.isEnabled(skill));
		if(index == this.index())hh = 18
        this.drawPic(skill.card, rect.x, rect.y-hh);
        this.changePaintOpacity(1);
    }
};
...
Window_BattleSkill.prototype.hitTest = function(x, y) {
    if (this.isContentsArea(x, y)) {
        ...
        var cy = y - this.padding;
        var topIndex = this.topIndex();
        for (var i = this.maxPageItems(); i >= 0; i--) {
            var index = topIndex + i;
            if (index < this.maxItems()) {
                var rect = this.itemRect(index);
                var right = rect.x + rect.width;
...
Window_BattleSkill.prototype.drawPic = function(faceName, x, y,ww,hh) {
    var bitmap = ImageManager.loadPictureCard(faceName);
	var w = ww || this.itemWidth();
	var h = hh || this.itemHeight();
    this.contents.blt(bitmap, 0, 0, w, h, x, y+18);
};
Window_BattleSkill.prototype.itemRect = function(index) {
    ...
    rect.width = this.itemWidth();
    rect.height = this.itemHeight();
    rect.x = index % maxCols * (rect.width + this.spacing()) - this._scrollX+5;
    rect.y = Math.floor(index / maxCols) * rect.height - this._scrollY;
    return rect;
};
