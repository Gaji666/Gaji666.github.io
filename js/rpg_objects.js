...
Game_Party.prototype.initCards = function() {
    //this._cards = [1,1,1,5,5,6,1,1,1,5,5,6,1,1,1,5,5,6,1,1,1,5,5,6];
	this._actskill = [];
	this._cards = [];
	this._Gcards = [];
	this._Qcards = [];	
	for(var i = 0;i<this.maxBattleMembers();i++){
		this._actskill.push.apply(this._actskill,this.battleMembers()[i].skills())
	}
	for(var i = 0;i<this._actskill.length;i++){
		this._cards.push(this._actskill[i].id)
	}
};
Game_Party.prototype.startCards = function() {
    var max = 4;
	for(var i = 0; i < max;i++){
		this.randomCards();
	}
};
Game_Party.prototype.randomCards = function() {
	if(this._cards.length <= 0) {
		this._cards.push.apply(this._cards,this._Qcards)
		this._Qcards = [];
	}	
    var ll = this._cards.length;
	var cc = this._cards[Math.randomInt(ll)]
	this._Gcards.push(cc)
	this._cards.splice(0,1);
};
Game_Party.prototype.useCards = function(index) {
	this._Qcards.push(this._Gcards[index])
	this._Gcards.splice(index,1);
};
Game_Party.prototype.useRandomCards = function(index) {
    var ll = this._Gcards.length;
	var cc = Math.randomInt(ll)	
	this.useCards(cc)
};
